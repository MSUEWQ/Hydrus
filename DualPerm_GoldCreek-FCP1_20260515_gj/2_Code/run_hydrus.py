...
def run_dual_perm(vgs, depths, calib, atm, atm_hydrus, obs, days, timesteps2, 
    ...
    for i in range(len(timesteps2)):
        ...
        #create soil profile
        profile = ph.create_profile(bot= -1*max(depths["EndDepth"]), #depth of soil profile
                                    dx= 1, #grid cells 1 cm
                                    h=-150 #intial pressure head
                                    )
        
        # Add materials for their specified depths
        for i in range(len(depths.index)):
            profile.loc[depths.loc[i,"StartDepth"]+1:depths.loc[i,"EndDepth"]+1, "Mat"] = i+1
    ...
    if ("FAILED.csv" in os.listdir(os.path.join(main_level, '3_Results', newfolder))) == False:
        ...
        for col in calib_cols: # loop through all columns we want for calibration
            cols_to_keep.append(col) # grab column from hydrus_ouput
        hydrus_output = alldata.loc[:,cols_to_keep] # ditch columns that aren't date or calib_cols
        ...
        # If run returns static values, replace all values with failure defaults from "settings"
        if len(np.unique(hydrus_output)) == 1:
            hydrus_output = calib.copy().drop(columns=["Date"])
            for col, val in zip(settings["FieldDataColumn"], settings["FailValue"]):
                hydrus_output[col] = val
            alldata = None

        if return_all == True:
            return alldata
        
        return np.array(hydrus_output, dtype=float).reshape(len(hydrus_output), -1)

    else: 
        print("Run failure")
        #return array with same shape as field calib data, filled with failure default valuess from "settings"
        failed_run = calib.copy().drop(columns=["Date"])
        for col, val in zip(settings["FieldDataColumn"], settings["FailValue"]):
            failed_run[col] = val
        
        return np.array(failed_run, dtype=float).reshape(len(failed_run), -1)
    


def run_single_por(vgs, calib, atm, atm_hydrus, obs, days, timesteps2, calib_cols, settings, return_all = False):
    ...
    if ("FAILED.csv" in os.listdir(os.path.join(main_level, '3_Results', newfolder))) == False:
        ...
        print("Run failure")
        #return array with same shape as field calib data, filled with failure default valuess from "settings"
        failed_run = calib.copy()
        for col, val in zip(settings["FieldDataColumn"], settings["FailValue"]):
            failed_run[col] = val
        return np.array(failed_run.drop(columns=["Date"]))
